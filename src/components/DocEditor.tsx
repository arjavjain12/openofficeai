...
export default function DocEditor({ id, initialData, canEdit }: DocEditorProps) {
  ...
  const save = useCallback(async () => {
    ...
    const api = univerRef.current.univerAPI as { getActiveDocument: () => { getSnapshot: () => unknown } | null }
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const enUS = (await import('@univerjs/preset-docs-core/locales/en-US')).default
    ...
    <div className="h-screen w-screen flex flex-col">
      <header className="h-12 border-b border-zinc-200 flex items-center px-4 justify-between bg-white shrink-0">
        <div className="flex items-center gap-3">
          <a href="/dashboard" className="font-semibold text-sm text-zinc-900">OpenOfficeAI</a>
          <span className="text-xs text-zinc-400">/ Document</span>
          {!canEdit && (
            ...
            {dlOpen && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1 bg-white border border-zinc-200 rounded-xl shadow-lg py-1 z-20 min-w-[130px]">
                  <a href={`/api/v1/download/${id}?format=pdf`} onClick={() => setDlOpen(false)} className="block px-3 py-2 text-xs hover:bg-zinc-50">PDF</a>
                  <a href={`/api/v1/download/${id}?format=docx`} onClick={() => setDlOpen(false)} className="block px-3 py-2 text-xs hover:bg-zinc-50">Word (.docx)</a>
                  <a href={`/api/v1/download/${id}?format=txt`} onClick={() => setDlOpen(false)} className="block px-3 py-2 text-xs hover:bg-zinc-50">Plain Text</a>
                </div>
              </>
            )}
          </div>
