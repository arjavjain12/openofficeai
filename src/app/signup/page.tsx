...
function SignupContent() {
  ...
  const then = searchParams.get('then') // 'sheet' or 'doc'
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (then) {
      // After signup, redirect to dashboard where they can create
    ...
    <div className="min-h-[100dvh] flex items-center justify-center bg-[var(--bg)] px-4">
      ...
      >
        ...
        <p className="text-sm text-zinc-500 mb-6">
          {then ? `Sign up to create your ${then === 'sheet' ? 'spreadsheet' : 'document'}` : 'Start creating docs and sheets via API'}
        </p>
        ...
        <p className="mt-6 text-center text-sm text-zinc-500">
          Already have an account? <a href={`/login${then ? '?then=' + then : ''}`} className="text-zinc-900 font-medium hover:underline">Log in</a>
        </p>
      </motion.div>
    </div>
  )
